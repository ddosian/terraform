...
resource "dockhand_environment" "ts-prod-02_dockhand_environment" {
  ...
  ca_cert         = local.ts-prod-02_ca
  client_cert     = local.ts-prod-02_cert
  client_key      = local.ts-prod-02_key
  icon            = "server"
}
...
resource "authentik_certificate_key_pair" "ts-prod-02-ca_authentik_key_pair" {
  name             = "ts-prod-02-ca"
  certificate_data = local.ts-prod-02_ca
  lifecycle {
...
resource "authentik_certificate_key_pair" "ts-prod-02-client_authentik_key_pair" {
  ...
  certificate_data = local.ts-prod-02_cert
  key_data         = local.ts-prod-02_key
  lifecycle {
    ignore_changes = [certificate_data, key_data]
  }
}
...
resource "authentik_provider_proxy" "traefik-ts-prod-02_authentik_provider" {
  name               = "Provider for Traefik (ts-prod-02)"
  external_host      = "https://traefik.ts-prod-02.internal.dontddos.me"
  mode               = "forward_single"
  authorization_flow = data.authentik_flow.explicit-authorization-flow.id
  invalidation_flow  = data.authentik_flow.default-provider-invalidation-flow.id
}

resource "authentik_application" "traefik-ts-prod-02_authentik_application" {
  name              = "Traefik (TS-Prod-02)"
  slug              = "traefik-ts-prod-02"
  protocol_provider = authentik_provider_proxy.traefik-ts-prod-02_authentik_provider.id
  meta_icon         = "https://cdn.jsdelivr.net/gh/homarr-labs/dashboard-icons/svg/traefik.svg"
  group             = "Networking"
}

resource "authentik_policy_binding" "traefik-ts-prod-02_lab-admins_authentik_policy_binding" {
  target = authentik_application.traefik-ts-prod-02_authentik_application.uuid
