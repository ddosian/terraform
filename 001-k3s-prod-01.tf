...
# Uptime Kuma Monitor
resource "uptimekuma_monitor_ping" "k3s-prod-01_uptimekuma_monitor" {
  name           = "K3s-Prod-01"
  hostname       = "k3s-prod-01.internal.dontddos.me"
  interval       = 60
  timeout        = 30
  max_retries    = 2
  retry_interval = 60
...
resource "uptimekuma_monitor_tcp_port" "k3s-prod-01_kube_api" {
  name        = "K3s-Prod-01 (Kube API)"
  hostname    = "k3s-prod-01.internal.dontddos.me"
  port        = 6443
  interval    = 60
  max_retries = 2
