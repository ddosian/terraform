...
resource "dockhand_environment" "media-prod-01_dockhand_environment" {
  ...
  ca_cert         = local.media-prod-01_ca
  client_cert     = local.media-prod-01_cert
  client_key      = local.media-prod-01_key
  icon            = "server"
}
...
resource "authentik_certificate_key_pair" "media-prod-01-ca_authentik_key_pair" {
  ...
  certificate_data = local.media-prod-01_ca
  lifecycle {
...
resource "authentik_certificate_key_pair" "media-prod-01-client_authentik_key_pair" {
  ...
  certificate_data = local.media-prod-01_cert
  key_data         = local.media-prod-01_key
}
