...
resource "dockhand_environment" "dns-prod-01_dockhand_environment" {
  ...
  ca_cert         = local.dns-prod-01_ca
  client_cert     = local.dns-prod-01_cert
  client_key      = local.dns-prod-01_key
  icon            = "server"
}
...
resource "authentik_certificate_key_pair" "dns-prod-01-ca_authentik_key_pair" {
  name             = "dns-prod-01-ca"
  certificate_data = local.dns-prod-01_ca
  lifecycle {
...
resource "authentik_certificate_key_pair" "dns-prod-01-client_authentik_key_pair" {
  ...
  certificate_data = local.dns-prod-01_cert
  key_data         = local.dns-prod-01_key
  lifecycle {
    ignore_changes = [certificate_data, key_data]
  }
}
