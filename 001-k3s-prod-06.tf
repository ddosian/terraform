...
# Uptime Kuma Monitor
resource "uptimekuma_monitor_ping" "k3s-prod-06_uptimekuma_monitor" {
  name           = "K3s-Prod-06"
  hostname       = "k3s-prod-06.internal.dontddos.me"
  interval       = 60
  timeout        = 30
  max_retries    = 2
  retry_interval = 60
...
resource "uptimekuma_monitor_tcp_port" "k3s-prod-06_kube_api" {
  name        = "K3s-Prod-06 (Kube API)"
  hostname    = "k3s-prod-06.internal.dontddos.me"
  port        = 6443
  interval    = 60
  max_retries = 2
