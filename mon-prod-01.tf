...
resource "dockhand_environment" "mon-prod-01_dockhand_environment" {
  ...
  ca_cert         = local.mon-prod-01_ca
  client_cert     = local.mon-prod-01_cert
  client_key      = local.mon-prod-01_key
  icon            = "server"
}
...
resource "authentik_certificate_key_pair" "mon-prod-01-ca_authentik_key_pair" {
  name             = "mon-prod-01-ca"
  certificate_data = local.mon-prod-01_ca
  lifecycle {
...
resource "authentik_certificate_key_pair" "mon-prod-01-client_authentik_key_pair" {
  ...
  certificate_data = local.mon-prod-01_cert
  key_data         = local.mon-prod-01_key
  lifecycle {
    ignore_changes = [certificate_data, key_data]
  }
}
...
resource "authentik_provider_proxy" "traefik-mon-prod-01_authentik_provider" {
  name               = "Provider for Traefik (mon-prod-01)"
  external_host      = "https://traefik.mon-prod-01.internal.dontddos.me"
  mode               = "forward_single"
  authorization_flow = data.authentik_flow.explicit-authorization-flow.id
  invalidation_flow  = data.authentik_flow.default-provider-invalidation-flow.id
}

resource "authentik_application" "traefik-mon-prod-01_authentik_application" {
  name              = "Traefik (Mon-Prod-01)"
  slug              = "traefik-mon-prod-01"
  protocol_provider = authentik_provider_proxy.traefik-mon-prod-01_authentik_provider.id
  meta_icon         = "https://cdn.jsdelivr.net/gh/homarr-labs/dashboard-icons/svg/traefik.svg"
  group             = "Networking"
}

resource "authentik_policy_binding" "traefik-mon-prod-01_lab-admins_authentik_policy_binding" {
  target = authentik_application.traefik-mon-prod-01_authentik_application.uuid
