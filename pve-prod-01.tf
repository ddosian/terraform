...
resource "adguard_rewrite" "pve-prod-01_wildcard_record" {
  domain = "*.pve-prod-01.internal.dontddos.me"
  answer = "pve-prod-01.internal.dontddos.me"
}
...
resource "authentik_certificate_key_pair" "pve-prod-01-ca_authentik_key_pair" {
  name             = "pve-prod-01-ca"
  certificate_data = local.pve-prod-01_ca
  lifecycle {
...
resource "authentik_certificate_key_pair" "pve-prod-01-client_authentik_key_pair" {
  ...
  certificate_data = local.pve-prod-01_cert
  key_data         = local.pve-prod-01_key
  lifecycle {
    ignore_changes = [certificate_data, key_data]
  }
}
...
resource "authentik_service_connection_docker" "pve-prod-01_authentik_service_connection" {
  name               = "pve-prod-01"
  url                = "https://pve-prod-01.internal.dontddos.me:2376"
  tls_verification   = authentik_certificate_key_pair.pve-prod-01-ca_authentik_key_pair.id
  tls_authentication = authentik_certificate_key_pair.pve-prod-01-client_authentik_key_pair.id
}

resource "authentik_outpost" "pve-prod-01_authentik_outpost" {
  name = "pve-prod-01"
  protocol_providers = [
    authentik_provider_proxy.traefik-pve-prod-01_authentik_provider.id
  ]
  config = jsonencode({
    log_level                        = "info"
    docker_labels                    = null
  ...
  service_connection = authentik_service_connection_docker.pve-prod-01_authentik_service_connection.id
  lifecycle {
