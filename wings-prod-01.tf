...
resource "dockhand_environment" "wings-prod-01_dockhand_environment" {
  ...
  ca_cert         = local.wings-prod-01_ca
  client_cert     = local.wings-prod-01_cert
  client_key      = local.wings-prod-01_key
  icon            = "server"
}
...
resource "authentik_provider_proxy" "traefik-wings-prod-01_authentik_provider" {
  name               = "Provider for Traefik (wings-prod-01)"
  external_host      = "https://traefik.wings-prod-01.internal.dontddos.me"
  mode               = "forward_single"
  authorization_flow = data.authentik_flow.explicit-authorization-flow.id
  invalidation_flow  = data.authentik_flow.default-provider-invalidation-flow.id
}

resource "authentik_application" "traefik-wings-prod-01_authentik_application" {
  name              = "Traefik (Wings-Prod-01)"
  slug              = "traefik-wings-prod-01"
  protocol_provider = authentik_provider_proxy.traefik-wings-prod-01_authentik_provider.id
  meta_icon         = "https://cdn.jsdelivr.net/gh/homarr-labs/dashboard-icons/svg/traefik.svg"
  group             = "Networking"
}

resource "authentik_policy_binding" "traefik-wings-prod-01_lab-admins_authentik_policy_binding" {
  target = authentik_application.traefik-wings-prod-01_authentik_application.uuid
