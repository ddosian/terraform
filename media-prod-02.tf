...
resource "authentik_service_connection_docker" "media-prod-02_authentik_service_connection" {
  name               = "media-prod-02"
  url                = "https://media-prod-02.internal.dontddos.me:2376"
  tls_verification   = authentik_certificate_key_pair.media-prod-02-ca_authentik_key_pair.id
  tls_authentication = authentik_certificate_key_pair.media-prod-02-client_authentik_key_pair.id
}

resource "authentik_outpost" "media-prod-02_authentik_outpost" {
  name = "media-prod-02"
  protocol_providers = [
    authentik_provider_proxy.traefik-media-prod-02_authentik_provider.id
  ]
  config = jsonencode({
    log_level                        = "info"
    docker_labels                    = null
  ...
  service_connection = authentik_service_connection_docker.media-prod-02_authentik_service_connection.id
  lifecycle {
